...
- [x] Probada una tool de solo lectura desde el tab Chat — el agente respondió correctamente usando MCP de Render
...
**Incidente de capacidad:** el servicio se reinició por exceder el límite de memoria de `plan: starter` (dashboard + gateway + MCP + Tool Gateway de Nous simultáneos). Se sube a `plan: standard` (más RAM) — coste mayor, confirmado explícitamente por el usuario. `render.yaml` actualizado y desplegado.

**Incidente de permisos:** el redeploy a `plan: standard` falló al iniciar el agente (`Permission denied: /opt/data/shared/nous_auth.lock`). Causa: `hermes portal login` se había ejecutado como **root** desde la Shell de Render (en vez de como el usuario `hermes`, que es quien corre el gateway), dejando `/opt/data/shared/` con dueño `root:root` y modo `0700` — el proceso del gateway no podía ni entrar al directorio. Arreglado con `chown -R hermes:hermes /opt/data/shared`. **Lección:** cualquier comando `hermes ...` ejecutado a mano vía Shell de Render debe correr como el usuario `hermes` (`gosu hermes hermes ...` o `su hermes -c "..."`), nunca como root directamente, para no dejar archivos con dueño incorrecto en el disco persistente.
